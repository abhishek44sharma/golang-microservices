...
func TestCreateRepoInvalidErrorInterface(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.EqualValues(t, http.StatusInternalServerError, err.StatusCode)
	assert.EqualValues(t, "invalid json resonse body", err.Message)
}
...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			StatusCode: http.StatusCreated,
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 123,"name": "golang-tutorial","full_name": "abhishek44sharma/golang-tutorial"}`)),
		},
	})
	...
	assert.EqualValues(t, "abhishek44sharma/golang-tutorial", response.FullName)
}
